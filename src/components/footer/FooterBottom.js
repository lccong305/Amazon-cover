import React from "react";
import { footerBottomItem } from "../../constants";

const FooterBottom = () => {
  return (
    <div className="w-full py-8 bg-footerBottom">
      <div className="max-w-5xl mx-auto">
        <div className="grid w-full grid-cols-2 gap-3 mx-auto text-gray-400 sml:grid-cols-3 ml-14 xl:ml-0 md:grid-col-4 xl:grid-cols-7 place-content-center">
          {footerBottomItem.map((item) => (
            <div className="cursor-pointer group" key={item._id}>
              <h3 className="w-24 font-semibold text-[12px] group-hover:underline text-[#DDD] leading-3 mb-[2px]">
                {item.title}
              </h3>
              <p className="w-24 tracking-tight text-[12px] text-[#999] group-hover:underline leading-3">
                {item.des}
              </p>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default FooterBottom;
